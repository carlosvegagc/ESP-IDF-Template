...
void main_task( void* param )
{
	...
	char * message = (char *)malloc(LEN_MESSAGES_DISP);
	if(message == NULL){
	...
	for ( ;; )
	{
        ...
        if (xQueueReceive(lora_receiver_queue, (void * )&in_lora_message, (portTickType)portMAX_DELAY)) {
			...
			sprintf( message, "[%d]:%s", _counter, in_lora_message);	
...
void app_main()
{
    ...
    lora_receiver_queue = xQueueCreate(1, sizeof(char *));
    ...
    display_queue = xQueueCreate(2, sizeof(char *));
	...
	xTaskCreate(lora_receiver_task, "lora_sender_task", 2000, NULL, 5, NULL);
